...
class CelestialObjectViewModels {
    ...
    func fetchCelestianById(id: String) async throws -> CelestialObject {
        let newURL = URL(string: "https://api.airtable.com/v0/appHJzJ80jhZ6fY9f/Planètes/\(id)")!
        ...
        do {
            let decoded = try decoder.decode(CelestialObjectResult.self, from: data)
            
            let planete = decoded.fields
            return planete
            
        }catch{
            print ("Echec du décodage : planete")
            throw error
        }
    }
}
